...
def not_bad(s):
 ...
 # notp = re.search('not', s)
  #badp = re.search('bad', s) 
  #if badp == None or notp == None or notp.end() > badp.start():
  #  return s
  #if notp.end() < badp.start():
  #  return s[:notp.start()] + 'good' + s[badp.end():]
...
def conta2(n):
  ...
  conta = 0
  for i in range(0, n):
    if re.findall('2', str(i)):
      conta += len(re.findall('2', str(i)))
  return conta
...
def inip2(n):
  count = 0
  value = 0
  while not str(value).startswith(str(n)):
    count += 1
    value = 2**count
  return count
